...
test("sessionFileRevision changes when entries are appended", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-revision-"));
  try {
    const filePath = join(dir, "session.jsonl");
    assert.equal(sessionFileRevision(filePath), "", "missing file has no revision");
    assert.equal(sessionFileRevision(null), "");
    assert.equal(sessionFileRevision(undefined), "");

    writeFileSync(filePath, '{"type":"session"}\n');
    const first = sessionFileRevision(filePath);
    assert.notEqual(first, "");
    assert.equal(sessionFileRevision(filePath), first, "stable while unchanged");

    writeFileSync(filePath, '{"type":"message"}\n', { flag: "a" });
    const second = sessionFileRevision(filePath);
    assert.notEqual(second, first, "appended entry changes the revision");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
